import { CloseOutlined, BankOutlined, BorderlessTableOutlined, CheckOutlined, DollarOutlined, ExclamationCircleOutlined, MoneyCollectOutlined, ThunderboltOutlined, TrophyOutlined } from "@ant-design/icons";
...
export default function CryptoDetailsPage(): JSX.Element | null {
  ...
  return <section className="crypto-details">
    <div className="crypto-details__top">
      ...
      <p>{coinName} live price in US Dollar (USD). View value statistics, market cap and supply.</p>
    </div>
    ...
    <div className="crypto-details__info">
      <div className="crypto-stats__info-container">
        <h1 className="crypto-stats__title--main">What is {coinName}?</h1>
        {parse(description)}
      </div>
  ...
  return (
    <div className="crypto-details__stats">
      <div className="crypto-stats">
        ...
        <p className="stats__desc">An overview showing the statistics of {coinName}, such as the base and quote currency, the rank, and All time high.</p>
        <Col span={24}>
      ...
      <div className="crypto-other-stats">
        <div className="crypto-stats">
          ...
          <p className="stats__desc">An overview showing more statistics of {coinName}, such as the number of exchanges, the total and circulating supply.</p>
          <Col span={24}>
            <div className="stats__card">
              <h3 className="stats-title"><MoneyCollectOutlined /> Number Of tickets</h3>
              <span className="stats-value">{numberOfTickets}</span>
            </div>
          </Col>
        </div>
        <Col span={24}>
        ...
        <Col span={24}>
          <div className="stats__card">
            ...
            <span className="stats-value">{isConfirmed ? <CheckOutlined /> : <CloseOutlined />}</span>
          </div>
        </Col>
        <Col span={24}>
          <div className="stats__card">
...
function LinksSection(props: { links: CoinLinks[], coinName: string }) {
  ...
  return (
    <div className="crypto-stats__links-container">
      <h1 className="crypto-stats__title--main">{coinName} Links</h1>{links.map(link =>
        <Col span={24}>
          <div className="stats__card">
            <h3 className='crypto-stats__info-link--title'>{link.type}</h3>
            <a className="crypto-stats__info-link" href={link.url}>{link.name}</a>
          </div>
        </Col>
      )}
    </div>)
